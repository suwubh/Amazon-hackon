...
## 9. Value cascade — the derived terminal-state waterfall (MT8)
...
## 10. Web console + photo upload + buyer storefront (MT9)
...
The seller dashboard reskinned to a wide return-rate table over the existing `/seller/returns`; PDP, radar, liquidity, diagnose, health-card, route, metrics screens were reused and widened, logic untouched. (MT9 runs **before** MT8 so MT8's cascade strip is built into the web `RouteScreen`, not the retired phone frame. §8's "phone frame" persona switch is now the web home + `WebShell`.)

## 11. Buyer-flow fixes + resell redesign (MT10)

MT10 fixes five buyer/ops/resell defects before the MT5 bulletproof pass. The iron rule holds — every new number traces to an endpoint (seed JSON + a thin route), never hardcoded JSX. The ⭐ spine (SL-001 grade→route→card→radar) is untouched.
...
**Per-instance caveat (for MT5):** returns, listings, and interests are in-memory per Lambda instance, like the cart/passport — they reset on a cold start and aren't shared across parallel instances. Acceptable for the demo (single warm instance; rock-solid on local uvicorn).

## 12. Playbook-gap closures + iron-rule fixes (MT12)

A batch of 12 smaller correctness/honesty/depth fixes. Iron rule held throughout (every on-screen number traces to an endpoint); the ⭐ SL-001 spine is regression-clean (`local_p2p +₹83` vs `warehouse −₹129`, confirmed on the deployed Function URL).
